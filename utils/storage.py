...
class RolloutStorage(object):
    ...
    def __init__(self, agent_type, num_agent, num_step, batch_size, num_obs, num_action, num_rec):
        self.obs = tr.zeros(num_agent, batch_size, num_step + 1, *num_obs)
        self.rew = tr.zeros(num_agent, batch_size, num_step, 1)
        self.ret = tr.zeros(num_agent, batch_size, num_step + 1, 1)
        self.act = tr.zeros(num_agent, batch_size, num_step, num_action)
        self.mask = tr.zeros(num_agent, batch_size, num_step + 1, 1) # If mask == 0, Terminal
        ...
        if agent_type == 'ac':
            ...
            self.logprob = tr.zeros(num_agent, batch_size, num_step, 1)
    ...
    def add(self, obss, acts, rews, masks, infos):
        ...
        for i in range(self.num_agent):
            self.obs[i, self.n, self.step + 1].copy_(obss[i])
            self.act[i, self.n, self.step].copy_(self.onehot[acts[i]].view(-1))
            self.rew[i, self.n, self.step].copy_(rews[i])
            self.mask[i, self.n, self.step + 1].copy_(masks[i])
            
            # For CPC
            if self.agent_type == 'ac':
                v, logprob, h, s_f, a_f = infos[i]
                self.h[i, self.n, self.step + 1].copy_(h.view(-1))
                self.logprob[i, self.n, self.step].copy_(logprob.view(-1))
                self.val[i, self.n, self.step].copy_(v.view(-1))
                self.s_feat[i, self.n, self.step].copy_(s_f.view(-1))
                self.a_feat[i, self.n, self.step].copy_(a_f.view(-1))
